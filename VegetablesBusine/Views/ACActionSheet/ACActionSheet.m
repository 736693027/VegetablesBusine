...
- (void)_didClickButton:(UIButton *)button {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(actionACSheet:didClickedButtonAtIndex:)]) {
        [_delegate actionACSheet:self didClickedButtonAtIndex:button.tag];
    }
...
- (void)_dismissView:(UITapGestureRecognizer *)tap {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(actionACSheet:didClickedButtonAtIndex:)]) {
        [_delegate actionACSheet:self didClickedButtonAtIndex:_otherButtonTitles.count];
    }
...
- (void)show {
    ...
    [UIView animateWithDuration:ACShowAnimateDuration animations:^{
        @strongify(self)
        self.darkShadowView.alpha = ACDarkShadowViewAlpha;
