...
- (void)deleteAsset{
    ...
    MLSelectPhotoAssets *asset = [self.deletePhotos objectAtIndex:self.currentPage];
    if(asset.isVideoType){
        AVURLAsset *avAsset = [[AVURLAsset alloc] initWithURL:asset.assetURL options:nil];
        Float64 duration = CMTimeGetSeconds(avAsset.duration);
        if(duration>10){
            [SVProgressHUD showErrorWithStatus:@"视频内存太大，不支持上传"];
            return;
        }
    }
    if([[self.deleteAssets allValues] count] == 0 || [self.deleteAssets valueForKeyPath:[NSString stringWithFormat:@"%ld",(self.currentPage)]] == nil){
